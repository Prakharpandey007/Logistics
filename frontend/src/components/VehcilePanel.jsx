      ...
      {['Pickupvans', 'MiniTrucks', 'Trucks','ContainerTrucks'].map((type) => (
        ...
        >
          ...
          <div className='ml-2 w-1/2'>
            <h4 className='font-medium text-base capitalize'>{type === 'Pickupvans' ? 'Pickupvans' : type === 'MiniTrucks' ? 'MiniTrucks' :type==="Trucks"?"Trucks":type==="Container Trucks"?'UberAuto':"Booking Ride"} <span><i className="ri-user-3-fill"></i>{type === 'moto' ? 1 : type === 'auto' ? 3 : 4}</span></h4>
            <h5 className='font-medium text-sm'>{type === 'moto' ? '3 mins' : '2 mins'} away</h5>
            <p className='font-normal text-xs text-gray-600'>{type === 'Pickupvans' ? 'Affordable Pickupvans Bookings' : type === 'MiniTrucks' ? 'Affordable Minitrucks Bookings' :type==='Trucks'?'Affordable Trucks Bookings':type==='Container Trucks'?'Affordable Container Trucks Rides':"Affordable Bookings"}</p>
          </div>
